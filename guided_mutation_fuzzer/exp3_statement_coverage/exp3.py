...
def run_mutation_coverage_fuzzer_statement_coverage(
    run_time_in_min, dir_path_git, seed_path
):
    timeout = 60 * run_time_in_min
    delete_coverage_files(dir_path=dir_path_git)
    exe_path = fuzz_compile(
        dir_path=dir_path_git, exec_name="guided_statement_mutation_fuzzer"
    )
    mutation_guided_fuzzer = MutationFuzzer_coverage(
        path_seeds=seed_path, min_mutations=1, max_mutations=3
    )
    runner_fuzzing = ProgramRunner(program=exe_path)
    ...
    while time.time() < timeout_start + timeout:
        (
            ...
            mutated_seed,
            coverage,
        ) = mutation_guided_fuzzer.run_statement_coverage(runner=runner_fuzzing)
