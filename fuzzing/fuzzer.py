...
from fuzzing.utils_fuzzing import (
...
class MutationFuzzer(Fuzzer):
    def __init__(self, path_seeds, min_mutations=2, max_mutations=10):
        ...
        self.path_mutated_seeds = os.path.join(
            os.path.abspath(os.path.join(path_seeds, os.pardir)), "in_mutated/"
        )
        print(path_seeds)
        print(self.path_mutated_seeds)
    ...
    def save_mutated_seed(self, in_mutaded_input_dir: bool):
        if in_mutaded_input_dir:
            with open(self.mutaded_name[self.seed_index], "w") as f:
...
class MutationFuzzer_coverage(MutationFuzzer):
    ...
    def run_line_coverage(self, runner):
        ...
        self.actual_coverage = ProgrammCoverageRunner.return_actual_line_coverage(
            runner
        )
    ...
    def run_branch_coverage(self, runner):
        ...
        """
        result, outcome, mutated_seed = super().run(runner)
        self.actual_coverage = ProgrammCoverageRunner.return_actual_branch_coverage(
            runner
        )
        self.append_seed_if_better_coverage(outcome)
        return result, outcome, self.mutated_seed, self.actual_coverage
    ...
    def run_statement_coverage(self, runner):
        ...
        """
        result, outcome, mutated_seed = super().run(runner)
        self.actual_coverage = ProgrammCoverageRunner.return_actual_statment_coverage(
            runner
        )
        self.append_seed_if_better_coverage(outcome)
        return result, outcome, self.mutated_seed, self.actual_coverage
    ...
    def append_seed_if_better_coverage(self, outcome):
        if (outcome == ProgrammCoverageRunner.PASS) and self.actual_coverage > max(
            self.coverages_seen
        ):
            # We have new coverage
            actual_coverage_formated = "{:.2f}".format(self.actual_coverage)
            file_name_with_file_format = os.path.basename(
                self.mutaded_name[self.seed_index]
            )
            file_format = file_name_with_file_format.split(".")[-1]
            if "_coverage" in file_name_with_file_format:
                file_name_with_file_format = file_name_with_file_format.split(
                    "_coverage_"
                )[0]
            file_name = file_name_with_file_format.split(".")[0]
            name = (
                file_name + "_coverage_" + actual_coverage_formated + "." + file_format
            )
...
class Grammar_Fuzzer(Fuzzer):
    def __init__(
        ...
        self.path_grammar_seed = os.path.join(
            os.path.abspath(os.path.join(dir_path, os.pardir)), "in_grammar/"
        )
