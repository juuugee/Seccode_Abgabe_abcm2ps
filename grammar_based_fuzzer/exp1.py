...
def run_grammar_fuzzer(run_time_in_min, dir_path_git, seed_path):
    ...
    grammar_fuzzer = Grammar_Fuzzer(grammar=SIMPLE_ABC_GRAMMAR, dir_path=seed_path, start_sym=START_SYMBOL)
    runner_fuzzing_grammar = ProgramRunner(program=exe_path)
    timeout_start = time.time()
    while time.time() < timeout_start + timeout:
        result, outcome, grammar_seed = grammar_fuzzer.run(runner=runner_fuzzing_grammar)
